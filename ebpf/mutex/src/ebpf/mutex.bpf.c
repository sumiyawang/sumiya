...
int futex_exit(struct syscall_trace_exit *ctx)
{
	...
	delta /= 1000000U;
        ...
        if (delta < 2) {
                __sync_fetch_and_add(&(histp->slots[0]), 1);
        } else if(delta < 4) {
                __sync_fetch_and_add(&(histp->slots[1]), 1);
        } else if(delta < 8) {
                __sync_fetch_and_add(&(histp->slots[2]), 1);
        } else if(delta < 16) {
                __sync_fetch_and_add(&(histp->slots[3]), 1);
        } else if(delta < 32) {
                __sync_fetch_and_add(&(histp->slots[4]), 1);
        } else if(delta < 64) {
                __sync_fetch_and_add(&(histp->slots[5]), 1);
        } else if(delta < 128) {
                __sync_fetch_and_add(&(histp->slots[6]), 1);
        } else if(delta < 256) {
                __sync_fetch_and_add(&(histp->slots[7]), 1);
        } else if(delta < 512) {
                __sync_fetch_and_add(&(histp->slots[8]), 1);
        } else if(delta < 1024) {
                __sync_fetch_and_add(&(histp->slots[9]), 1);
        } else if(delta < 2048) {
        /*
         * fatal error for waiting
         */
                __sync_fetch_and_add(&(histp->slots[10]), 1);
        } else if(delta < 4096) {
                __sync_fetch_and_add(&(histp->slots[11]), 1);
        } else {
                __sync_fetch_and_add(&(histp->slots[13]), 1);
        }
